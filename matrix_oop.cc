...
S21Matrix S21Matrix::operator*(const double num) {
  if (rows_ == 0 || cols_ == 0)
    throw std::logic_error(
        "Error: You can't mul matrix if its rows or cols are 0");
...
void S21Matrix::MulMatrix(const S21Matrix &other) {
  if (cols_ != other.rows_)
    throw std::logic_error(
        ...
        "matrix.");
  if (!cols_ || !rows_ || !other.cols_ || !other.rows_)
    throw std::logic_error(
        "Error: You can't mul matrixes if rows or cols of one of them are 0");
...
bool S21Matrix::CheckNullptr() noexcept { return matrix_ == nullptr; }
...
void S21Matrix::MoveMatrix() noexcept {
  rows_ = 0;
  cols_ = 0;
  matrix_ = nullptr;
}
